...
format_error(#{error_type  := improper_tail,
               line        := LINE,
               actual_type := Ty}) ->
...
format_error(#{error_type    := non_integer_argument_to_bnot,
               line          := P,
               expected_type := Ty}) ->
...
format_error(#{error_type := record}) ->
    %%% TODO: Improve quality of type error
    "Type error in record";
format_error(#{error_type    := record,
               line          := P,
               expression    := Record,
               expected_type := ResTy}) ->
