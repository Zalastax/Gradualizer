...
-type type_error() ::
    #{error_type := type_error
                    ...
                    | string | int | float | improper_tail | cons_pat
                    ...
                    | non_number_argument_to_plus | non_number_argument_to_minus
                    | non_boolean_argument_to_not
                    | unary_op | non_integer_argument_to_bnot
                    | logic_op | {rel_op, arguments | expression}
                    | {list_op, arguments | expression}
                    | operator_pattern | pattern | tuple
                    | {bit, arguments | expression} | generator | bin_generator
                    | check_clauses | record | record_pattern | record_update
...
type_check_expr(Env, {cons, LINE, Head, Tail}) ->
            ...
            TailElemTy =
                case Ty2 of
                    ...
                    {type, _, L, [TailElemTy0]} when L == list;
                        ...
                        TailElemTy0;
                    _ ->
                        throw({type_error, list, LINE, Ty2})
...
do_type_check_expr_in(Env, ResTy, {record_field, P, Expr, Record, {atom, _, Field}} = E) ->
    Rec = maps:get(Record, Env#env.tenv#tenv.records),
    FieldTy = get_rec_field_type(Field, Rec),
...
handle_type_error({type_error, non_integer_argument_to_bnot, P, Ty}) ->
    #{error_type    => non_integer_argument_to_bnot,
      line          => P,
      expected_type => Ty
      };
...
handle_type_error({type_error, record}) ->
    %%% TODO: Improve quality of type error
    #{error_type    => record};
